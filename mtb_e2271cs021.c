    ...
    uint8_t* frameBuffer, char* string, mtb_e2271cs021_font_t* fontInfo, uint8_t* fontCoordinates)
{
    ...
    for (currentChar = string;  *currentChar != '\0'; currentChar++)
    {
            ...
            (fontCoordinates[MTB_E2271CS021_FONT_Y] < (fontInfo->ySpan)))
        {
            ...
            printLocation = (fontInfo->xOffset) + (fontCoordinates[MTB_E2271CS021_FONT_X]   *
                            (fontInfo->xSize))  + (((fontCoordinates[MTB_E2271CS021_FONT_Y] *
                            (fontInfo->ySize))  + (fontInfo->yOffset))    *
                             PV_EINK_HORIZONTAL_SIZE);

            /* Check if the current character is a printable ASCII character
               between "!" and "~" */
            if ((*currentChar <= MTB_E2271CS021_ASCII_MAX) && (*currentChar >= MTB_E2271CS021_ASCII_MIN))
            {
                     ...
                     verticalCounter++)
                {
                         ...
                         horizontalCounter++)
                    {
                        ...
                        fontByte = charLocation + (verticalCounter * (fontInfo->xSize)) + horizontalCounter;

                        /* Copy the pixel data for background, invert for white background */
                        frameBuffer[frameByte] = ((fontInfo->color) == MTB_E2271CS021_WHITE_BACKGROUND)
